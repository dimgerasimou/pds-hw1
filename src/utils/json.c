 ...
 */
static int
parse_int(const char **p, int *value)
{
    skip_whitespace(p);
    char *end;
...
parse_matrix_info(const char *json, MatrixInfo *info)
{
    ...
    if (find_key(&p, "path") && !parse_string(&p, info->path, sizeof(info->path)))
        return 0;
    if (find_key(&p, "rows") && !parse_int(&p, &info->rows))
        return 0;
    if (find_key(&p, "cols") && !parse_int(&p, &info->cols))
        return 0;
    if (find_key(&p, "nnz") && !parse_int(&p, &info->nnz))
        return 0;
...
parse_benchmark_info(const char *json, BenchmarkInfo *info)
{
    ...
    if (find_key(&p, "threads") && !parse_int(&p, &info->threads))
        return 0;
    if (find_key(&p, "trials") && !parse_int(&p, &info->trials))
        return 0;
...
parse_result(const char *json, Result *result)
{
    ...
    if (find_key(&p, "algorithm") && !parse_string(&p, result->algorithm, sizeof(result->algorithm)))
        return 0;
    if (find_key(&p, "connected_components") && !parse_int(&p, &result->connected_components))
        return 0;
